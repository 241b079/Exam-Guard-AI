...
import { QuestionImportPreviewResponse, QuestionImportRow } from '../types';
...
export const ImportQuestionsModal: React.FC<ImportQuestionsModalProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await questionService.previewImport(examId, selectedFile);
      setPreview(result);
    } catch (err: any) {
  ...
  const handleCommitImport = async () => {
    ...
    try {
      ...
      await questionService.commitImport(examId, validRows);
      onImported();
      onClose();
    } catch (err: any) {
          ...
          {preview && (
            <div className="flex gap-3">
              ...
              <Button
                ...
                isLoading={isLoading}
              >
                Import {preview.valid_count} Valid Questions
              </Button>
            </div>
          )}
        </div>
